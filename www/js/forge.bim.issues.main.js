    ...
    success: function (res) {  
     ...
     if(res!=null && res != ''){ 
       ...
       if (n.type === 'bim360projects')  
           ...
           //project name maybe contains chars that are not accepted as DOM id 
           //var itemId = n.text.replace(/\s/g, '');

           //switch to project id (removing b.) 
           var params = n.id.split('/');
           var project_id_with_b = params[params.length - 1];
           var project_id_without_b = project_id_with_b.replace('b.','')

           $('#project-list-tab-'+projViewId).append(
              '<a class="list-group-item list-group-item-action" ' + 
              'id="project-'+ project_id_without_b  + '"' +
           ...
           $('#project-' + project_id_without_b).data('bim-project-id', n.id);
           $('#project-' + project_id_without_b).click(function(){
